...
git remote add origin git@github.com:yizhishang/tools.git
...
1. git 配置：
git config --global user.name "yizhishang"      -- 配置用户名，上传本地 repository 到服务器上的时候，在 Github 上会显示这里配置的上传者信息
git config --global user.email "285206405@qq.com"     -- 配置邮箱
...
6. 克隆远程仓库
	git clone git@github.com:yizhishang/tools.git tools
	git init
	git remote add origin git@github.com:yizhishang/tools.git